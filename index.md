...
Use [`tidync()`](https://docs.ropensci.org/tidync/reference/tidync.md)
and
[`hyper_filter()`](https://docs.ropensci.org/tidync/reference/hyper_filter.md)
to discern what variables and dimensions are available, and to craft
axis-filtering expressions by value or by index. (Use the name of the
variable on the LHS to target it, use its name to filter by value and
the special name `index` to filter it by its index).
...
Please note that
[`hyper_filter()`](https://docs.ropensci.org/tidync/reference/hyper_filter.md)
expressions must be unique, unlike with
